...
export function ClientForm({ initialData, onCancel, refreshClients, readOnly = false }: ClientForm & { readOnly?: boolean }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(
        initialData ? `http://localhost:5001/api/clients/${initialData.id}` : "http://localhost:5001/api/clients",
        {
  ...
  const addComplianceField = () => {
    setComplianceFields([...complianceFields, { key: "", type: "boolean", value: false }])
  }
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-8 bg-white p-8 rounded-xl shadow-sm">
      <div className="flex items-center justify-between border-b border-gray-200 pb-6">
        <h2 className="text-2xl font-bold text-gray-900">{initialData ? "Edit Client Details" : "New Client"}</h2>
        <button
          type="button"
